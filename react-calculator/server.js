...
/**
The following routes forward requests (using pipe) from our React client to our dapr-enabled services. Our Dapr sidecar lives on localhost:<daprPort>. We invoke other Dapr enabled services by calling /v1.0/invoke/<DAPR_ID<SERVICE'S_ROUTE>.
*/
...
app.post('/calculate/subtract', async (req, res) => {
  try {
      const serviceUrl = process.env.services__subtractapp__http__0;
      const appResponse = await axios.post(`${serviceUrl}/subtract`, req.body);
      // Return expected string result to client
      return res.send(`${appResponse.data}`); 
  } catch (err) {
      console.log(err);
...
app.post('/calculate/multiply', async (req, res) => {
  try {
      const serviceUrl = process.env.services__multiplyapp__http__0;
      
      const appResponse = await axios.post(`${serviceUrl}/multiply`, req.body);
...
app.post('/calculate/divide', async (req, res) => {
  try {
      const serviceUrl = process.env.services__divideapp__http__0;

      const appResponse = await axios.post(`${serviceUrl}/divide`, req.body);
